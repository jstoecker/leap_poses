#ifndef __GL_MATH_VECTOR3_H__
#define __GL_MATH_VECTOR3_H__
...
namespace gl
{
	...
	struct Vector3
	{
		...
		// Constructs a 3D vector by copying the first two values from v and using z as the third.
		Vector3<T>(const Vector2<T>& v, float z) : x(v.x), y(v.y), z(z) {}

		// Constructs a 3D vector by copying the values from v.
		Vector3<T>(const Vector3<T>& v) : x(v.x), y(v.y), z(v.z) {}

		// Constructs a 3D vector by copying the xyz values from v
